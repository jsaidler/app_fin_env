...
class IntegrationApiSuite(unittest.TestCase):
    ...
    def test_03_recurrence_support_user(self) -> None:
        ...
        st, _, raw = self.api.call("PUT", f"/api/recurrences/{rid}", token=self.user_token, payload={"type": "out", "amount": 76, "category": "Gastos", "description": "suite-rec-updated-no-account", "frequency": "weekly", "start_date": today, "next_run_date": today, "account_id": None}); self.assertEqual(st, 200, raw)
        st, d, raw = self.api.call("GET", f"/api/recurrences/{rid}", token=self.user_token); self.ok(st, raw); self.assertIn(int(d.get("account_id", 0)), (0,), raw)
        st, _, raw = self.api.call("DELETE", f"/api/recurrences/{rid}", token=self.user_token); self.assertEqual(st, 200, raw)
        st, _, raw = self.api.call("DELETE", f"/api/recurrences/{rid_no_account}", token=self.user_token); self.assertEqual(st, 200, raw)
