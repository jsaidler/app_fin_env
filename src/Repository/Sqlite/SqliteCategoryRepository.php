...
class SqliteCategoryRepository implements CategoryRepositoryInterface
{
    ...
    {
        $now = date('c');
        $stmt = $this->pdo->prepare('INSERT INTO categories (name, type, alterdata_auto, created_at, updated_at) VALUES (:name,:type,:auto,:created,:updated)');
        $stmt->execute([
            'name' => $name,
            'type' => $type,
            'auto' => $alterdataAuto,
    ...
    public function update(int $id, array $data): ?Category
    {
        ...
        $stmt = $this->pdo->prepare('UPDATE categories SET name=:name, type=:type, alterdata_auto=:auto, updated_at=:updated WHERE id=:id');
        $stmt->execute([
            'name' => $merged['name'],
            'type' => $merged['type'],
