...
class UserAccountService
{
    ...
    public function __construct(
        ...
        ?UserCategoryRepositoryInterface $userCategories = null
    ) {
    ...
    public function createForUser(int $userId, array $input): array
    {
        ...
        if (!Validator::nonEmpty($name) || !in_array($type, ['bank', 'card'], true)) {
            Response::json(['error' => 'Dados de tag invalidos'], 422);
        }
        $existing = $this->accounts->findByUserAndName($userId, $name);
        if ($existing) {
            Response::json(['error' => 'Nome ja existe em suas tags'], 409);
        }
        $this->assertColorAvailable($userId, $color);
        $created = $this->accounts->create($userId, $name, $type, $icon, $color, $initialBalance);
    ...
    public function updateForUser(int $id, int $userId, array $input): array
    {
        ...
        if (!$existing) {
            Response::json(['error' => 'Tag nao encontrada'], 404);
        }
    ...
    private function assertColorAvailable(int $userId, string $color, ?int $ignoreAccountId = null): void
    {
        foreach ($this->accounts->listByUser($userId, true) as $item) {
            if ($ignoreAccountId !== null && (int)$item->id === $ignoreAccountId) {
                continue;
            }
    ...
    private function normalizeInitialBalance($value): float
    {
        if (is_string($value)) {
            ...
            if (!is_numeric($normalized)) {
                Response::json(['error' => 'Saldo inicial invalido'], 422);
            }
